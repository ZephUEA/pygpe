...
def chartCorrelator( psi, scalars, fileName, scale = False ) -> None:
    ...
    times = list( filter( lambda x: x < averageCorrelation.shape[ 1 ], [ i for i in range( 0 ) ] ) )
    firstZeroValues = []
    for time in times:
        if scale:
            ...
            plt.plot( handle_array( xs / divider ), handle_array( averageCorrelation / averageCorrelation[0,:] )[:,time] )
            plt.xlim( 0, 6 )
    ...
    plt.cla()
    if scale:
        mindex = min( enumerate( firstZeroValues ), key = lambda x: x[1] )[0]
        xs = cp.array( [ ( time - zeroTime )* ( scalars["dt"] * scalars["frameRate"] ) for time in times[mindex:] ] )
        plt.plot( xs, firstZeroValues[mindex:] )
        ( b, m ) = corr.bestFitLine( cp.log10(xs), cp.log10(firstZeroValues[mindex:]) )
        plt.plot( xs,  10**b * xs**m )
...
def main( recalculate:bool=False ):
    ...
    fileName = 'kzm_2d_t_instant.hdf5'
    ...
    # if not cupyImport:
    #     chartName = 'correlators/kzm_correlator_t900PrimeV5.png'
